...
class AdminerSimpleMenu
{
    ...
    public function tablesPrint($tables)
    {
        if (defined("PMTN_ADMINER_THEME")) {
            ...
            echo "<ul id='tables' class='simple'>" . Adminer\script("mixin(qs('#tables'), {onmouseover: menuOver, onmouseout: menuOut});");
        }
        ...
        foreach ($tables as $table => $status) {
            ...
            if ($this->preferSelect || Adminer\support("table") || Adminer\support("indexes")) {
                echo '<a href="' . Adminer\h(Adminer\ME) . $action . '=' . urlencode($table) . '"' . Adminer\bold($active, (Adminer\is_view($status) ? "view" : "")) . "' data-link='main' data-main='true'>$name</a>";
            } else {
    ...
    public function selectLinks($tableStatus, $set = "")
    {
        ...
        if ($this->preferSelect) {
            $links["select"] = Adminer\lang('Select data');
        }
        ...
        foreach ($links as $key => $val) {
            echo " <a href='" . Adminer\h(Adminer\ME) . "$key=" . urlencode($tableStatus["Name"]) . ($key == "edit" ? $set : "") . "'" . Adminer\bold(isset($_GET[$key])) . ">$val</a>";
        }
